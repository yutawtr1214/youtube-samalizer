...
def mock_generate_summary():
    ...
    # src.cli内でimportされているgenerate_summaryをpatchする
    ...
    with runner.isolated_filesystem():
        with patch.dict('os.environ', {'GEMINI_API_KEY': 'dummy_key'}):
            ...
            mock_generate_summary.assert_called_once_with(
                ...
                lang='en',
                prompt='Explain like I am five',
                stream=False # streamオプションが指定されていない場合のデフォルト値
            )
...
def test_missing_api_key(runner):
    ...
    with runner.isolated_filesystem():
        # 環境変数からGEMINI_API_KEYを削除
        with patch.dict('os.environ', {}, clear=True):
            result = runner.invoke(main, ['https://www.youtube.com/watch?v=dQw4w9WgXcQ'])

            assert result.exit_code != 0 # 0以外の終了コードを期待
            # エラーメッセージにAPIキーに関する言及があるか確認
...
def test_stream_mode(runner, mock_generate_summary):
    """ストリーミングモードのテスト"""
    with runner.isolated_filesystem():
        with patch.dict('os.environ', {'GEMINI_API_KEY': 'dummy_key'}):
            result = runner.invoke(main, [
            ...
            assert result.exit_code == 0
            assert '=== 要約生成中 ===' in result.output # ストリーミング開始メッセージ
            mock_generate_summary.assert_called_once_with(
                url='https://www.youtube.com/watch?v=dQw4w9WgXcQ',
                model='gemini-2.0-flash',
                length='normal',
                ...
                lang='ja',
                prompt=None,
                stream=True # stream=Trueが渡されることを確認
            )